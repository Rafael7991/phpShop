...
scene.add( camera );
message.changeMessage("Posição do boneco: ( UP: " + fwdValue  + ", LEFT:" + lftValue + ", RIGHT: " + rgtValue + " BACK: "  + ")" );
}
...
function update()
{
  ...
  if(lftValue > 0)
  {
    ...
    else
    {
      if(select == true)
      {
        cube2.position.x += 0.15;
        man.position.x += 0.15;
        objectSel.position.x += 0.15;
        cube2.position.z += 0.15;
        man.position.z += 0.15;
        objectSel.position.z += 0.15;
        man.rotation.set(0,0.7,0);
        cube2.rotation.set(0,0.7,0);
  ...
  if(fwdValue > 0 || rgtValue > 0 || bkdValue > 0 || rgtValue > 0)
  {
    control = true;
  } else {
    control = false;
